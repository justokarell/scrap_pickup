...
from .models import CollectionLocation, GeoPoint, ProcessingSite, Scenario
...
def _point(
    obj: CollectionLocation | ProcessingSite | GeoPoint,
    point_type: str,
) -> dict[str, Any]:
...
def _osrm_trip(
    ...
    return {
        "distance_miles": float(trip["distance"]) * MILES_PER_METER,
        ...
        if quantity > truck.max_weight_lbs + 1e-9:
            state.failures.append(
                Failure(
                    ...
                    message=(
                        f"{location.name} contains {quantity:.2f} lbs, "
...
def _best_processing_trip(
    ...
    collection_points = [
        _point(locations[location_id], "collection")
        for location_id in batch
    ]
    ...
    for site in scenario.processing_sites:
        ...
        available_storage = (
            site.storage_capacity_lbs - site_state.inventory_lbs
        )

        if batch_weight > available_storage + 1e-9:
            continue
    ...
    return min(
        candidates,
        key=lambda item: item[1]["distance_miles"],
    )
...
def execute_greedy_route(
    ...
    try:
        route_origin = _point(scenario.depot, "depot")
    except Exception as exc:
        state.failures.append(
            Failure(
                timestamp_hours=timestamp_hours,
                type="geocoding_failure",
                message=f"Depot geocoding failed: {exc}",
                truck_id=truck.id,
            )
        )
        return
    ...
    ordered_stops: list[dict[str, Any]] = [route_origin]
    total_miles = 0.0
    ...
    for batch in batches:
        ...
        for stop in collection_stops:
            ...
            if (
                ...
                state.failures.append(
                    Failure(
                        ...
                        type="truck_weight_capacity",
                        message=(
                            "Route would exceed truck capacity at "
                            f"{location.name}."
                        ),
                        location_id=location_id,
                        truck_id=truck.id,
            ...
            total_service_minutes += location.service_minutes
        ...
        if truck_state.load_lbs > available_storage + 1e-9:
            raise RuntimeError(
                "Processing storage became insufficient after route planning."
            )

        unload_quantity = truck_state.load_lbs

        site_state.inventory_lbs += unload_quantity
        site_state.received_lbs += unload_quantity
        ...
        current_origin = _point(site, "processing_site")

    if total_collected <= 1e-12:
        return

    # Every executed route returns to the explicit scenario depot.
        ...
        route_segments.append(
            {
                ...
                "stops": [current_origin, route_origin],
            }
        )
        ordered_stops.append(route_origin)
